...
/**
* Go through our top posters (or comments!), and assign their posts.
*/
function addPostsToTopPosters(top_posters, posts) {
	...
	for (var k in top_posters) {
		...
		poster.num_likes = 0;
		for (var i in poster.posts) {

			var post = poster.posts[i];

			// 
			// Total up the number of likes for this user.
			//
			poster.num_likes += post.like_count;

			//
			// If threre is an attachment, try to inline it
			//
			if (post.attachment) {

				var type = post.attachment.type;
				var url = post.attachment.media.image.src;

				if (type == "photo" || type == "sticker") {
					post.message += "Photo/sticker:<br/>" 
					 	+ "<a href=\"http://www.facebook.com/" + post.id + "\">"
						//
						// Hardcode the height and width for now to something sane.
						// In the future, I could try writing code to reduce this proporitionately.
						//
		...
		//
		// Sort by recent posts/comments first
		//
		poster.posts.sort(function(a, b) {
...
/**
* Go through the data structure of recent posts for a group, get top posters,
* top commenters, and all related data.
*
* @param object config Our configuration
* @param object group_data Metadata on the group itself
* @param object data Our data for the recent group posts from Facebook
*
* @return object Stats on that group
*/
function processPostData(config, group_data, data) {

	var post_stats = new stats();
	var comment_stats = new stats();

	//
	// Store our posts and comments so that we can pair them to top posters 
	// (and commenters) later.
	//
	var posts = {};
	var comments = {};
	...
	for (var key in data.data) {
		...
		//
		// If we have user info, add this post into our posts object.
		...
		if (from) {
			...
			post.like_count = 0;
			if (post.likes) {
				post.like_count = post.likes.data.length;
			}
			delete post.likes;
	...
	addPostsToTopPosters(top_commenters, comments);
...
function fetchGroupData(tokens, crowd_key, crowd_name, group_key, config) {
	...
	tokens.get().then(function(data) {
		...
		//
		// We don't want to index groups that are not open.
		//
		//data.privacy = "foobar"; // Debugging
		if (data.privacy != "OPEN") {

			if (data.privacy == "CLOSED" && !config.allowClosedGroups) {
				console.log("# ");
				console.log("# Group ID '" + config.id + "': Sorry, but I cannot currently create leaderboards on groups which are not open.");
				console.log("# This is because the access token of a user of this app who does not have access to the group might be used.");
				console.log("# If you'd like to support added, please go to https://github.com/dmuth/facebook-group-leaderboard and file a bug. :-)");
				console.log("# ");
				throw new Error("Unable to create leaderboard for group ID '" + config.id + "' (" + data.name + ")");
			}
		...
		//
		var uri = id + "/feed?fields=from,created_time,message,story,picture,full_picture"
			+ ",comments{created_time,from,message,like_count,attachment},likes"
			+ "&limit=" + config.numPosts;

		return queryAsync(uri, token, config.retryInterval);

	}).then(function(data) {
		var stats_data = processPostData(config, group_data, data);
		stats_data.crowd_name = crowd_name;
		...
		//console.log("ERROR STACK", error.stack); // Debugging
...
function getGroupConfig(config, group) {
	...
	var retval = {
		...
		allowClosedGroups: config.allowClosedGroups,
		};
