...
obj.prototype.getData = function() {
	...
	//
	// First step, put the data into an array, which we can then sort.
	// Note that we're storing the object key in the array
	//
	var to_sort = [];
	for (var key in facebook_data) {
		var row = facebook_data[key];
		row.old_key = key;
		to_sort.push(row);
	}
	...
	//
	// Now sort the array, first by name, then by ascending posts.
	//
	to_sort.sort(function(a, b) {
		...
		if (a.name < b.name) {
			...
			if (a.config.numPosts < b.config.numPosts) {
				return(-1);

			} else if (a.config.numPosts > b.config.numPosts) {
				return(1);
	...
	//
	// Finally, create a new object based on the sorted array.
	// I'm not 100% certain if sort order will be preserved--if this
	// becomes a problem in production, I'll revisit this appraoch and probably just
	// create an element in the original object with the keys in sorted order.
	//
	retval = {};
	for (var key in to_sort) {
		var row = to_sort[key];
		var target_key = row.old_key;
		delete row.old_key;
		retval[target_key] = row;
