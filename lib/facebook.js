...
*/
function fetchGroupData(token, group_id, config) {
	...
	query(uri, token, config.retryInterval, function(err, data) {
		...
		query(uri, token, config.retryInterval, function(err, data) {
			...
			debug("Refreshing this data again in " + config.refreshInterval + " seconds... " + uri);
			setTimeout(function() {
				fetchGroupData(token, group_id, config);
...
obj.prototype.go = function() {
	...
	for (key in groups) {
		...
		setImmediate(function() {
			...
			fetchGroupData(token, group.id, group_config);
			});
