	...
	query(uri, token, config.retryInterval, function(err, data) {
		...
		query(uri, token, config.retryInterval, function(err, data) {
			...
			if (config.userPhotos) {
				...
				fetchGroupDataTopUserPictures(token, config, stats_data.post_stats, function(err, post_stats) {
					stats_data.post_stats = post_stats;

					fetchGroupDataTopUserPictures(token, config, stats_data.comment_stats, function(err, comment_stats) {
